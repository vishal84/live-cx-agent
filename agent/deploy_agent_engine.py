...
async def send_message(prompt: str):
    sessions = await local_agent.async_create_session(user_id="cx_user")
...
asyncio.run(send_message("Whats the date and time?"))

#%%
# Deploy on Agent Engine

vertexai.init(project=os.getenv("GOOGLE_CLOUD_PROJECT"), staging_bucket="gs://kpmg-agents-agent-engine")
...
async def remote_send_message(prompt: str):
    session = await deploy_agent.async_create_session(user_id="cx_user")
